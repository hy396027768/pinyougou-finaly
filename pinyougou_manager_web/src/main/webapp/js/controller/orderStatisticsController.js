 //控制层 
app.controller('orderStatisticsController' ,function($scope,$filter,$controller,orderService,sellerService){
    ...
    $scope.searchDefined = function () {
        ...
        // $scope.startTime = $filter("date")($scope.startTime,"yyyy-MM-dd HH:mm:ss");
        // $scope.endTime = $filter("date")($scope.endTime,"yyyy-MM-dd HH:mm:ss");
        $scope.findOrderService();
    }
    //查询所有已审核的商家
    $scope.findSellerNameList = function () {
        sellerService.findAll().success(
        	function (response) {
				$scope.sellerNameList = response;
            }
		)
    }
    //当商家id发生改变时发送查询
    $scope.$watch("sellerName",function (newVlaue,oldValue) {
        $scope.findOrderService();
    });
});	
